...
export function Panels() {
  ...
  return (
    <aside className="absolute right-5 top-20 bottom-20 w-[380px] max-w-[calc(100vw-40px)] bg-white/95 backdrop-blur-xl border border-gray-200 rounded-2xl p-5 overflow-y-auto pointer-events-auto shadow-lg flex flex-col gap-4 text-xs font-medium text-gray-700 z-10 transition-all">

      {/* Close button */}
      <button
        onClick={() => backToAssembly()}
        className="absolute top-3 right-3 text-gray-400 hover:text-gray-700 p-1 rounded-md hover:bg-gray-100 transition-colors"
        title="Close panel"
      >
        <X className="w-4 h-4" />
      </button>
